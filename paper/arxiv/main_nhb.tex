...
than expected under conventional finite-order Markov models \citep{ebeling1994entropy, lin2017critical, montemurro2002longrange}. A conditional counterpart, Hilberg's conjecture --- a
reinterpretation of Shannon's guessing experiments \citep{shannon1951prediction} --- posits that
the conditional entropy of text likewise falls as a power law of context length
\citep{hilberg1990grenzwert, debowski2015relaxed}.

None of these measures, however, isolates the order in which units appear. Unit frequencies are
order-blind by construction; long-range mutual information can reflect repeated lexical and
topical content even when the local ordering of that content is disrupted; and conditional
entropy, although conditioned on preceding text, tracks total predictive information, in which
the contributions of word identities and of their arrangement are combined and cannot be
separated. Yet the same words at the same separations can compose entirely different meanings
...
context step by step, farther and farther back, makes that influence a function of distance $d$. At
each step we ask how much the revealed context reduces the probe's surprise, and subtract the
