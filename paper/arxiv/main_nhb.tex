...
leave-one-sentence-out influence matrices, 15 documents per corpus; positions supported by at
least 40 sentence pairs shown; error bars, s.e.m.). TED residuals are flat across positions.
Fiction shows a small elevation at the earliest positions (${\sim}0.05$, an order of magnitude
below its mean per-sentence influence), consistent with the added cost of removing sentences that
...
examined by absolute position; positions supported by fewer than 40 pairs are not displayed.
