...
large language models as probabilistic probes, we isolated order-specific influence with a
shuffled-token baseline; the contextual persistence function $P(d)$ measures it at distance $d$. Across ten corpora (449 documents) spanning six language
...
reproducibility of its exponent, not the power-law form alone: Zipf's sits near unity across a wide
range of languages and corpora.

Both describe the units in aggregate, independent of the order in which they appear, and so occupy
the most basic level of a hierarchy of statistical structure, in which each successive level is
more specific and is defined by removing the contribution of the one below. A second level concerns
not how often units occur but how they depend on one another across distance. That language carries such long-range dependence is established: the mutual information
between symbols falls off as an approximate power law of their separation, decaying far more slowly
than expected under conventional finite-order Markov models \citep{ebeling1994entropy, lin2017critical, montemurro2002longrange}. But this measure
of raw statistical correlation conflates genuinely sequential structure with order-independent
effects such as shared vocabulary and topic: long-range mutual information can reflect repeated
lexical and topical content even when the local ordering of that content is disrupted. It therefore
does not isolate the contribution of order itself.

A third level lies beyond both, and no law has yet captured it: the order-specific dependence that
remains once the contribution of unit identity is removed. This is not a formal residue. Word order
...
qualitatively rather than as a quantitative decay law. Whether this order-specific dependence obeys a
scaling law, and if so of what form, is unknown.
...
previously undescribed quantitative law governing contextual persistence in human language:
the evidence establishes a robust regularity, and the principal question it raises is why it
exists.
...
scrambled order (the lower rungs of the hierarchy above), so $P(d)$ isolates the component of contextual dependence
that survives only when sequential order is preserved, resolved as a function of distance.
...
consistency of the exponent across such diverse samples makes this a candidate broadly
...
dependence that remains once unit identity and co-occurrence are removed. We read it
as a law to be explained rather than as an adjudication between mechanisms, and its main force is
as a constraint. The evidence establishes a robust regularity; the principal question is why it
exists. Any successful account of how human language is generated must explain why its statistical
