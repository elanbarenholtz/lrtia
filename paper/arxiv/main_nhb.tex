...
What the data do not establish is that human production is implemented by a sequential process
drawing directly on arbitrarily distant prior material; that mechanistic claim the present
measurements cannot adjudicate, and we do not make it. The shape of the law is nonetheless
suggestive. A scale-free, order-specific persistence structure is what a process that generates
each word against its own accumulated ordered history would be expected to leave in its product,
and the law can be read as a candidate trace of such autoregressive organization in human
generation. Modern language models offer no independent test of this reading: they are explicitly
autoregressive, but they are trained on human text and inherit its statistics, so reproducing the
law would show only that they have learned it. The open question is the law's origin: whether the
near-unity exponent is a generic consequence of autoregressive generation, or a signature of the
human system --- most plausibly its memory constraints --- that artificial models inherit through
the data. The convergence of the exponent with model-free measurements of human memory favours
the second reading. Distinguishing the two cleanly --- for example, by measuring the CPF in
autoregressive models trained on sequences whose persistence structure differs from language's ---
is a direct target for future work.
