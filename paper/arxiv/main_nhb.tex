...
as a law to be explained rather than as an adjudication between mechanisms: a robust empirical
regularity whose primary role is to constrain any account of how human language is generated. Such an
account must explain why its statistical product lacks a detectable characteristic scale over this range. An architecture with a fixed maintenance
...
history of past use, with exponents near $0.8$.
\citet{kahana2002age}, examining associative decay in free recall via the lag-conditional
response probability, reported a forward power-function exponent of $0.82 \pm 0.09$ for younger
adults. Neither measurement involves a language model. That two model-free measurements of human memory land in the same order-unity exponent
...
The convergence extends to the mathematical form of forgetting itself: across several human and
animal memory tasks, forgetting is better described by a power function than by exponential and
other alternatives \citep{wixted1991form}. That the order-specific structure of language should
decay at a near-unity power-law rate, matching the decay of the memory systems that must encode
and retrieve it, raises the possibility (a hypothesis, not a finding) that linguistic structure is
organized to align with, or exploit, the natural decay characteristics of human memory, so that
the accessibility of prior content falls in step with its predictive value. On this ecological
reading, the CPF describes the statistical structure of the linguistic input stream, and a memory
system rationally adapted to that structure \citep{anderson1991reflections} would mirror its decay
rather than impose an arbitrary one --- the temporal analogue of the visual system's tuning to the
$1/f$ spatial statistics of natural scenes \citep{field1987relations, simoncelli2001natural}.

This picture aligns with a longer trajectory in cognitive psychology away from architecturally
bounded views of memory and toward continuous, graded characterizations: Cowan's
embedded-processes account \citep{cowan2001magical}, Ericsson and Kintsch's long-term working
memory \citep{ericsson1995longterm}, and continuous-resource models of visual working memory
\citep{ma2014changing}. The CPF provides a direct quantitative measurement of an analogous
continuity in natural language, at the scale of token-level sequential dependence in naturalistic
discourse. To the extent the law bears on this debate, it is consistent with that trajectory.
