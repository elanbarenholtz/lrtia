...
including the earliest positions in the prior context (Figure~\ref{fig:distributed}b). Persistence is therefore a distributed field of influence across the
...
for any specific position. \textbf{b,} Mean residual per-sentence influence by absolute position in the
prior context, after regressing influence on log(distance) (leave-one-sentence-out influence
matrices, 15 documents per corpus; positions supported by at least five sentence pairs shown; error
bars, s.e.m.). Immediately adjacent pairs (one sentence back) are excluded: adjacent-sentence
influence systematically exceeds the log-linear distance model, so their residuals reflect adjacency
rather than position (Methods). Residuals lie within ${\pm}0.05$ of zero at all positions, including
the earliest context position (pos $=0$, dotted line): influence is graded by recency, with no
privileged anchor positions.}
...
(standardized 25-sentence prior-context windows). For the positional analysis, per-sentence influence
was regressed on log(distance) and mean residuals were examined by absolute position. Immediately
adjacent sentence pairs (distance of one sentence) were excluded from this analysis: adjacent-sentence
influence systematically exceeds the log-linear distance model, so including these pairs would
conflate adjacency with position --- in particular, the final context positions, which pair with the
target region only at the nearest distances, would otherwise inherit the model's short-range misfit.
Positions supported by at least five pairs are reported. Variation in influence magnitude was
accounted for by distance, with no detectable residual effect of structural position once distance
was controlled.
