...
is needed in natural environments \citep{anderson1991reflections} and associative decay in free
recall \citep{kahana2002age} both follow power laws with exponents near $0.8$, and forgetting
itself, across human and animal tasks, is better described by power functions than by
exponentials \citep{wixted1991form}. The exponents are not identical, and we do not claim a
single mechanism generates them all; but the recurrence of the scale-free, order-unity form
across paradigms that are unambiguously about memory indicates that it is neither an artefact of
our instrument nor idiosyncratic to language.

This convergence raises the possibility (a hypothesis, not a finding) that linguistic structure
is organized to exploit the natural decay characteristics of human memory: a memory system
rationally adapted to the statistics of the linguistic stream \citep{anderson1991reflections}
would mirror its decay, as the visual system is tuned to the $1/f$ statistics of natural scenes
\citep{field1987relations, simoncelli2001natural}. It is likewise consistent with the longer
trajectory in cognitive psychology away from architecturally bounded memory toward continuous,
graded characterizations \citep{cowan2001magical, ericsson1995longterm, ma2014changing}, for
which the CPF supplies a direct quantitative measurement in naturalistic language. In each case
the observation doing the work is the same one: a near-$1/d$ persistence law whose near-unity
exponent places language at the boundary where each logarithmic timescale contributes
approximately equally to prediction.
